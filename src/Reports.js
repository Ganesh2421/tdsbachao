...
const rendertable = (drivers, index) => {
	...
	localStorage.setItem('user_name_report',JSON.stringify(drivers.user_name));
	localStorage.setItem('user_key_report',JSON.stringify(drivers.user_key));
  localStorage.setItem('org_key_report',JSON.stringify(drivers.org_key));
  localStorage.setItem('org_name_report',JSON.stringify(drivers.org_name));
  
}
		return(
		<tr key = {index}>
			
			<td>{drivers.user_name}</td>
			<td>{drivers.org_name}</td>
			<td><Link to ="/generate_reports">< Button variant="primary" onClick = {Details}>Click</Button></Link></td>
		</tr>
			);
		}
...
<ReactTable
          ...
          columns={[
                ...
                {
                  Header: "Click to upload Reports",
                  Cell: ({ cell }) => (
                    <Link to ='/generate_reports'><Button >
                                  Click Here
                    </Button></Link>)
                }
          ...
          getTrProps={(state, rowInfo,column, instance) => {
            return {
              onClick: e =>{
                ...
                localStorage.setItem('user_name_report',JSON.stringify(rowInfo.original.user_name));
                localStorage.setItem('user_key_report',JSON.stringify(rowInfo.original.user_key));
                localStorage.setItem('org_key_report',JSON.stringify(rowInfo.original.org_key));
                localStorage.setItem('org_name_report',JSON.stringify(rowInfo.original.org_name));
